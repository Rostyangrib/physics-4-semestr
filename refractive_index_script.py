        ...
        if not os.path.exists(databasePath) and auto_download:
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                urllib.request.urlretrieve(
                    f'https://github.com/polyanskiy/refractiveindex.info-database/archive/{_DATABASE_SHA}.zip',
                    zip_filename)
                print("extracting...", file=sys.stderr)
                with zipfile.ZipFile(zip_filename, 'r') as zf: zf.extractall(tempdir)
                shutil.move(os.path.join(tempdir, f"refractiveindex.info-database-{_DATABASE_SHA}", "database"),
                            databasePath)
                            ...
                            for p in b['content']:
                                if 'DIVIDER' not in p:
                                    if p['PAGE'] == page:
                                        ...
                                        filename = os.path.join(self.referencePath, 'data-nk',
...
class FormulaRefractiveIndexData:
    ...
    def getRefractiveIndex(self, wavelength, bounds_error=True):
        ...
        if self.rangeMin <= np.min(wavelength) <= self.rangeMax and self.rangeMin <= np.max(
                wavelength) <= self.rangeMax or not bounds_error:
...
class TabulatedRefractiveIndexData:
    ...
    def getRefractiveIndex(self, wavelength, bounds_error=True):
        ...
        elif self.rangeMin <= np.min(wavelength) <= self.rangeMax and self.rangeMin <= np.max(
                wavelength) <= self.rangeMax and self.rangeMin != self.rangeMax or not bounds_error:
